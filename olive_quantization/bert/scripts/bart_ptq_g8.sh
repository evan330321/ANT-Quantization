...
mkdir -p ./log/bart_${size}_ptq_g8/$task_name
...
python run_glue.py \
  ...
  --output_dir ./log/bart_${size}_ptq_g8/$task_name/ \
  --mode $mode \
  --abit 4 \
  --wbit 4 \
  -wu 250 \
  -wl 75  \
  -au 250 \
  -al 75 --group_size 8 2>&1 | tee ./log/bart_${size}_ptq_g8/$task_name/${batch_size}.log