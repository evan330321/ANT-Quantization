...
mkdir -p ./log/bert_${size}_ptq_w2a4_g8/$task_name
...
python run_qa.py \
  ...
  --output_dir ./log/bert_${size}_ptq_w2a4_g8/$task_name/ \
  --mode $mode \
  --abit 4 \
  --wbit 2 \
  -wu 250 \
  -wl 75  \
  -au 250 \
  -al 75 --group_size 8 2>&1 | tee ./log/bert_${size}_ptq_w2a4_g8/$task_name/${batch_size}.log
